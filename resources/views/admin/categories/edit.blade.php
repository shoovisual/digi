...
@if(session('status'))
    <div class="mb-3 p-3 bg-green-50 text-green-700 border border-green-200 rounded">{{ session('status') }}</div>
@endif
@if($errors->any())
...
<form action="{{ route('admin.categories.update', $category) }}" method="post" enctype="multipart/form-data" class="card p-4 space-y-4">
    @csrf
    @method('PUT')
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" class="form-control" placeholder="e.g., bi bi-tv" />
        </div>
        ...
        <div>
            ...
            <input type="text" name="slug" value="{{ old('slug', $category->slug) }}" id="category-slug-input" class="form-control opacity-60" disabled />
            <div class="form-check mt-1">
                <input type="checkbox" name="manual_slug" id="category-slug-manual" class="form-check-input" />
        ...
        <div>
            ...
            <div class="mt-2">
                ...
                <img src="{{ $img }}" alt="{{ $category->name }}" class="w-32 h-20 object-cover rounded border" />
            </div>
        </div>
    </div>
